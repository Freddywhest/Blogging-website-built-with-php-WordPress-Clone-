<? require_once("vendors/h-header.php"); ?>
<body <? echo $theme_name == "b-style"?"class='boxed-layout'":"class='layouter'"; ?><? echo $theme_name == "b-dark"?"class='boxed-layout'":""; ?>>

	<? require_once("vendors/cat-nav.php"); ?>
	<?
	if(isset($_GET['hot_post_id'])){
		$sql = "UPDATE hot_news SET hot_post_views = hot_post_views +1 WHERE hot_post_id =:id";
	...
	<div class="page-title">
		<div class="container">
			...
			$hot_post_url = $post['hot_post_url'];
			$hot_post_user_id = $post['hot_post_user_id'];
			$hot_post_user_id = $post['hot_post_user_id'];
			$hot_post_detail = $post['hot_post_detail'];
			$hot_post_image = $post['hot_post_image'];
			$hot_post_date = $post['hot_post_date'];
			$hot_post_status = $post['hot_post_status'];
			$hot_post_author = $post['hot_post_author'];
			$hot_post_views = $post['hot_post_views'];
			$hot_post_tags = $post['hot_post_tags'];
			$hot_post_comment_count = $post['hot_post_comment_count'];
			$hot_post_category_id = $post['hot_post_category_id'];
				...
				<div class="col-md-12">
					...
					<div class="single-post">
						...
						<div class="post-title-area">
							<a class="post-cat" href="../../category/<? echo $hot_post_category_id; ?>/<? echo $cat_url; ?>"><? echo $cat_name; ?></a>
							<h2 class="post-title">
				 				<? echo $hot_post_title; ?>
				 			</h2>
				 			<div class="post-meta">
								<span class="post-author">
									By <a href="../../author/<? echo $hot_post_user_id; ?>/<? echo $hot_post_author; ?>"><? echo $hot_post_author; ?></a>
								</span>
								<? if($u_reg == 'Yes'){ ?>
								<span class="post-date"><i class="fa fa-clock-o"></i> <? echo $hot_post_date; ?></span>
								<? }else{ ?>
								<? } ?>
								<span class="post-hits"><i class="fa fa-eye"></i> <? echo $hot_post_views; ?></span>
								<span class="post-comment"><i class="fa fa-comments-o"></i>
								<a href="#comments" class="comments-link"><span><? echo $hot_post_comment_count; ?></span></a></span>
							</div>
						</div><!-- Post title end -->

						<div class="post-content-area">
							<div class="post-media post-featured-image">
								<img src="../../uploads/<? echo $hot_post_image; ?>" class="img-fluid" alt="<? echo $hot_post_title; ?>">
							</div>
							<div class="entry-content">
								<p><? echo $hot_post_detail; ?></p>
							...
							<div class="share-items clearfix">
   							<ul class="post-social-icons unstyled">
   								<li class="facebook form-group">
									<a href="<? echo $url; ?>/h-detail/<? echo $_GET['hot_post_id']; ?>/<? echo $hot_post_url; ?>" 
									  onclick="
										window.open(
